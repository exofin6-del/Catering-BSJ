...
function MapLocateControl({
    ...
    async function startLocating() {
        ...
        try {
            ...
            // Fly the map directly to the geolocated coordinates
            map.flyTo(latlng, Math.max(map.getZoom(), 18), {
                animate: true,
                duration: 1.0,
            })

            onLocationFound?.(latlng, position.coords.accuracy)
        } catch (error) {
            setLocation(null)
            setIsLocating(false)
    ...
    return (
        <MapControlContainer className={cn(position, className)}>
            <Button
                ...
                onClick={handleClick}
                disabled={isLoading}
                title={isLoading ? 'Mencari lokasi...' : 'Gunakan lokasi saat ini'}
                aria-label={isLoading ? 'Mencari lokasi...' : 'Gunakan lokasi saat ini'}
                className="border">
                {isLoading ? (
                    <LoaderCircleIcon className="animate-spin" />
                ) : (
                    <LocateFixed className="size-5 text-muted-foreground/45 group-data-[selected]:text-primary" />
                )}
            </Button>
